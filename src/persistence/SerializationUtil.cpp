...
string SerializationUtil::toSerealizeForm(uint8_t value8) {
    ...
    LOG(INFO) << "spyCBlock::SerializationUtil: " << "The form convertion uint16 with endian tipe is: "
              << formLiteEndian;
...
string SerializationUtil::toSerealizeForm(DVarInt valueVarInt) {
    ...
    *stream << std::hex << valueVarInt.getValue();
    ...
    LOG(INFO) << "spyCBlock::SerializationUtil: " << "The form convertion varint with hexadecimal form is: "
              << hexConvert;
    //delete stream;

    unsigned int dimensioVarInt = GetSizeOfCompactSize(valueVarInt.getValue());
    LOG_IF(ERROR, dimensioVarInt < 0)
                    << "Error in the readed dimension of compact size, the dimension eraded is: " << dimensioVarInt;
