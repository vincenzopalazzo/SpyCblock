...
void TransactionOutput::toTransactionsGraph(ofstream &outputStream, spyCBlockRPC::WrapperInformations &wrapper) {
    wrapper.addInformationLink(spyCBlockRPC::WrapperInformations::TypeInsert::TRANSACTION,
                               "amount: " + to_string(this->nValue)); //Not is the ammount of transaction
}
...
bool
TransactionOutput::isScriptNull() { //This isn't null but containt the OP_RETURN, before introduce it, some transaction are null.
