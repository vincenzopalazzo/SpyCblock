...
void RawTransaction::decode(std::ifstream& stream) {
  ...
  if (numberTxIn.getValue() == 0) {
    LOG(WARNING) << "Found the marker";

    this->numberTxIn.decode(stream);

    if (numberTxIn.getValue() == 1) {
      LOG(WARNING) << "Found the flag";
      type = Type::WITNESS;
      numberTxIn.decode(
          stream);  // With this code I readed the number
                    // tranaction input in Witnees transaction type;
    }
  } else {
    type = Type::PRIMITIVE;
  }

  LOG(INFO) << "Number transaction input in raw transaction "
            << numberTxIn.getValue();
  txIn.clear();
  ...
  LOG(INFO) << "Number transaction output in raw transaction "
            << numberTxOut.getValue();
  ...
  this->hashRawTransaction =
      CryptoSingleton::getInstance().getHash256(serializationForm);
...
string RawTransaction::toSerealizationForm() const {
  ...
  hexForm.append(SerializationUtil::toSerializeForm(this->numberTxIn));
  ...
  hexForm.append(SerializationUtil::toSerializeForm(this->numberTxOut));
  ...
  hexForm.append(SerializationUtil::toSerializeForm(this->lockTime));
  // Debug hex
...
// Getter and setter
uint8_t RawTransaction::getFlag() const { return flag; }

uint8_t RawTransaction::getMarker() const { return marker; }
...
uint32_t RawTransaction::getLockTime() const { return lockTime; }
