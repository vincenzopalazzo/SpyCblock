...
Sha256Hash Sha256::getHash() {
  ...
  append(&temp, 1);
  temp = 0x00;
  while (bufferLen != 56) append(&temp, 1);
  for (int i = 0; i < 8; i++) {
...
Sha256Hash Sha256::getHmac(const uint8_t key[], size_t keyLen,
  ...
  // Compute inner hash
  for (int i = 0; i < BLOCK_LEN; i++) tempKey[i] ^= 0x36;
  const Sha256Hash innerHash =
      Sha256().append(tempKey, BLOCK_LEN).append(msg, msgLen).getHash();

  // Compute outer hash
  for (int i = 0; i < BLOCK_LEN; i++) tempKey[i] ^= 0x36 ^ 0x5C;
  return Sha256()
...
void Sha256::compress(uint32_t state[8], const uint8_t block[BLOCK_LEN]) {
  ...
  for (int i = 0; i < NUM_ROUNDS; i++) {
    uint32_t t1 = 0U + h + (rotr32(e, 6) ^ rotr32(e, 11) ^ rotr32(e, 25)) +
                  (g ^ (e & (f ^ g))) + ROUND_CONSTANTS[i] + schedule[i];
    uint32_t t2 = 0U + (rotr32(a, 2) ^ rotr32(a, 13) ^ rotr32(a, 22)) +
                  ((a & (b | c)) | (b & c));
    h = g;
    g = f;
    f = e;
