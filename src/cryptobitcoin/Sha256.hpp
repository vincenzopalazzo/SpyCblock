...
/*
 * Computes the SHA-256 hash of a sequence of bytes, returning a Sha256Hash
 * object. Provides three static methods, and an instantiable stateful hasher.
 */
class Sha256 final {
  /*---- Public constants ----*/

 public:
  static constexpr int BLOCK_LEN = 64;  // In bytes
 ...
 public:
  static Sha256Hash getHmac(const std::uint8_t key[], std::size_t keyLen,
                            const std::uint8_t msg[], std::size_t msgLen);

 public:
  static void compress(std::uint32_t state[8],
                       const std::uint8_t block[BLOCK_LEN]);

  // Requires 1 <= i <= 31
 private:
  static std::uint32_t rotr32(std::uint32_t x, int i);

  /*---- Private constants ----*/

 private:
  static constexpr int NUM_ROUNDS = 64;

 private:
  static const std::uint32_t ROUND_CONSTANTS[NUM_ROUNDS];
};
