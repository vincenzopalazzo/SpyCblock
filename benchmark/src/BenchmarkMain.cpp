...
int main() {
  ...
  const int logLevel =
      spyCBlock::ConfiguratorSingleton::getInstance().getLevelLog();
  ...
  std::vector<std::string> typeDeserializzation = {
      "uncompressed-tx", "compresse-tx", "json-uncompressed"};
  std::vector<int> directory = {10, 20, 30};
  int iterationrForBenchmark = 5;
  std::string root = std::experimental::filesystem::current_path();

  cppproperties::Properties prop =
      cppproperties::PropertiesParser::Read(root + "/conf.properties");
  ...
  while (size_t(type) < typeDeserializzation.size()) {
    ...
    while (size_t(onData) < directory.size()) {
      ...
      int parallel =
          true; // TODO change this to false, this is setted to true because now
                // I want data only for testing the paralel esecution
      ...
      while (i < 5 || parallel == false) {
        ...
        if (typeAnlisis == "uncompressed-tx") {
          ...
          std::string name = std::to_string(numFile) + "-" + typeAnlisis +
                             (parallel == false ? "" : "-parallel");
          resultBenchmark.setNameBenchmark(name);
          cout << "Iteration " << i + 1 << " for benchmark " << name << endl;
          if (parallel) {
            auto start = std::chrono::system_clock::now();
            spyCBlock.convertDataParallel<spyCBlock::DAOTransactionsGraph>(
                dao, fromDir, toDir);
            ...
            auto start = std::chrono::system_clock::now();
            spyCBlock.convertData<spyCBlock::DAOTransactionsGraph>(dao, fromDir,
                                                                   toDir);
            auto end = std::chrono::system_clock::now();
            std::chrono::minutes minutes =
                std::chrono::duration_cast<std::chrono::minutes>(end - start);
          ...
          if (parallel) {
            auto start = std::chrono::system_clock::now();
            spyCBlock.convertDataParallel<spyCBlock::DAOJson>(dao, fromDir,
                                                              toDir);
            auto end = std::chrono::system_clock::now();
            std::chrono::minutes minutes =
                std::chrono::duration_cast<std::chrono::minutes>(end - start);
...
void displayInformation() {
  cout << "\n\n\nCopyright (c) 2018-2019 Vincenzo Palazzo "
          "vicenzopalazzodev@gmail.com\n";
  cout
      << "Distributed under the Apache License Version 2.0 software license,\n";
...
void toJson(std::vector<spyCBlock::ResultBenchMark> &results) {
  std::string pathOut =
      "result/json/scalability-benckmark-{YOUR-DETAILS-CPU-COMPILER}.json";
